    ...
    def run(self, state: TradingState):
        ...
        for product, order_depth in state.order_depths.items():
            ...
            if product == "KELP":
                ...
                if len(self.price_history_kelp) < 5:
                    continue
                
                sma_5 = np.mean(self.price_history_kelp[-5:])
                stdev_5 = np.std(self.price_history_kelp[-5:])

                
                if mid_price > (sma_5 + 2*stdev_5): 
                    orders.append(Order(product, best_ask, -5)) 
                    print("SOLD KELP") 

                elif mid_price < (sma_5 - 2*stdev_5): 
